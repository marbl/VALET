...
A second assumption is that the DNA sequence is a read originates from a contigous region of the genome.  An indicator of a potential mis-assembly is if two ends of a reads align to different regions of the assemby.
...
__PICK UP REVISIONS HERE__

Amosvalidate~\citep{amosvalidate2008} is a \emph{de novo} pipeline for detecting mis-assemblies incorporating the above constraints. REAPR~\citep{hunt2013reapr} is a tool that leverages insert size constraints and evaluates the accuracy of the assembly using read-pair coverage.
...
Contigs within the metagenomic assemblers vary widely in abundances __REF__.
