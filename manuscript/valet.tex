...
Though QUAST is able to calculate a number of quality metrics, such as \emph{ADD Examples}, without a reference genome, these metrics only provide information about the overall quality of the assembly not assembly accuracy.
...
VALET takes as input a metagenome assembly \textsc{FASTA} file and a collection of paired and un-paired reads  (Figure \emph{REF SUPPLEMENTAL})%(Figure~\ref{fig:valet_pipeline}).
...
For example, lets look at one of the suspicious regions flagged in the HMP assembly (Figure \emph{REF SUPPLEMENTAL}).%\ref{fig:hmp_plasmid} .
